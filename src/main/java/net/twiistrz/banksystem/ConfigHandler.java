...
public final class ConfigHandler {
  ...
  public String printBalance(Player p1, String message, Player p2) {
    String format = "#,##0.00";
    if (this.plugin.getConfigurationHandler().getString("Settings.moneyFormat").equals("#,###.##") || this.plugin
      .getConfigurationHandler().getString("Settings.moneyFormat").equals("#,###") || this.plugin
      .getConfigurationHandler().getString("Settings.moneyFormat").equals("#.##") || this.plugin
      .getConfigurationHandler().getString("Settings.moneyFormat").equals("#,##0.00") || this.plugin
      .getConfigurationHandler().getString("Settings.moneyFormat").equals("#,##0") || this.plugin
      .getConfigurationHandler().getString("Settings.moneyFormat").equals("0.00"))
      format = this.plugin.getConfigurationHandler().getString("Settings.moneyFormat"); 
    DecimalFormat money = new DecimalFormat(format);
    Double bankBalance = Double.valueOf(0.0D);
    Double totalBalance = Double.valueOf(BankSystem.econ.getBalance((OfflinePlayer)p2));
    if (this.plugin.getMoneyDatabaseInterface().hasUserdata(p2)) {
      bankBalance = this.plugin.getMoneyDatabaseInterface().getBalance(p2);
      totalBalance = Double.valueOf(BankSystem.econ.getBalance((OfflinePlayer)p2) + bankBalance.doubleValue());
    } 
    return message
      .replace("%balance%", this.plugin.getConfigurationHandler().getString("Settings.currency") + money.format(bankBalance))
      .replace("%pocket%", this.plugin.getConfigurationHandler().getString("Settings.currency") + money.format(BankSystem.econ.getBalance((OfflinePlayer)p2)))
      .replace("%total%", this.plugin.getConfigurationHandler().getString("Settings.currency") + money.format(totalBalance));
  }
  ...
  public void printMessage(Player p1, String messageKey, String amount, Player p2, String p2Name, Boolean prefix) {
    ...
    if (this.plugin.getConfig().contains(messageKey)) {
      ...
      if (((String)message.get(0)).contains("%interestCooldown%"))
        message.set(0, ((String)message.get(0)).replaceAll("%interestCooldown%", this.plugin.getInterestHandler().getNextInterestTime())); 
      if (amount != null && !amount.isEmpty())
        message.set(0, ((String)message.get(0)).replace("%amount%", this.plugin.getConfigurationHandler().getString("Settings.currency") + money.format(Double.parseDouble(amount)))); 
      if (prefix.booleanValue()) {
        p1.sendMessage(parseFormattingCodes(getString("Messages.prefix") + (String)message.get(0)));
      } else {
      ...
      p1.sendMessage(parseFormattingCodes(getString("Messages.prefix")) + "&cCould not locate " + messageKey + " in the config file inside of the " + this.plugin.getDescription().getName() + " folder! (Try generating a new one by deleting the current)");
    } 
  }
  ...
  public void printMessage(CommandSender sender, String messageKey, String amount, Player p2, String p2Name, Boolean prefix) {
    ...
    if (this.plugin.getConfig().contains(messageKey)) {
      ...
      if (amount != null && !amount.isEmpty())
        message.set(0, ((String)message.get(0)).replace("%amount%", this.plugin.getConfigurationHandler().getString("Settings.currency") + money.format(Double.parseDouble(amount)))); 
      if (prefix.booleanValue()) {
        sender.sendMessage(parseFormattingCodes(getString("Messages.prefix") + (String)message.get(0)));
      } else {
      ...
      BankSystem.logger.log(Level.SEVERE, this.noConfig, new Object[] { messageKey, this.plugin.getDescription().getName() });
      sender.sendMessage(parseFormattingCodes(getString("Messages.prefix")) + "&cCould not locate " + messageKey + " in the config file inside of the " + this.plugin.getDescription().getName() + " folder! (Try generating a new one by deleting the current)");
    } 
  }

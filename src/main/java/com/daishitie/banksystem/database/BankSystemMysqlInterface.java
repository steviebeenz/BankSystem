...
public class BankSystemMysqlInterface implements UserdataDatabaseInterface<Double> {
    ...
    public boolean hasUserdata(Player player) {
        ...
        try {
            String sql = "SELECT `player_uuid` FROM `" + this.plugin.config().getString("DataSource.mySQLTablename") + "` WHERE `player_uuid` = ? LIMIT 1";
    ...
    public boolean createUserdata(Player player) {
        ...
        try {
            String sql = "INSERT INTO `" + this.plugin.config().getString("DataSource.mySQLTablename") + "`(`player_uuid`, `player_name`, `money`, `last_seen`, `sync_complete`) VALUES(?, ?, ?, ?, ?)";
    ...
    public Double getBalance(Player player) {
        ...
        ResultSet result = null;
        try {
            String sql = "SELECT `money` FROM `" + this.plugin.config().getString("DataSource.mySQLTablename") + "` WHERE `player_uuid` = ? LIMIT 1";
            preparedUpdateStatement = conn.prepareStatement(sql);
            preparedUpdateStatement.setString(1, player.getUniqueId().toString());
            result = preparedUpdateStatement.executeQuery();
            if (result.next())
    ...
    public Double getBalance(UUID playerUUID) {
        ...
        ResultSet result = null;
        try {
            String sql = "SELECT `money` FROM `" + this.plugin.config().getString("DataBase.mySQLTablename") + "` WHERE `player_uuid` = ? LIMIT 1";
            preparedUpdateStatement = conn.prepareStatement(sql);
            preparedUpdateStatement.setString(1, playerUUID.toString());
            result = preparedUpdateStatement.executeQuery();
            if (result.next())
    ...
    public boolean setBalance(Player player, Double amount) {
        ...
        PreparedStatement preparedUpdateStatement = null;
        try {
            String updateSql = "UPDATE `" + this.plugin.config().getString("DataSource.mySQLTablename") + "` SET `money` = ?WHERE `player_uuid` = ?";

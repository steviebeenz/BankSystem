...
public class InterestHandler {
    ...
    public String getNextInterestTime() {
        ...
        long timecountSec = (this.plugin.config().getInteger("Settings.interestCooldown").intValue() * 60) - timecountSecPass;
    ...
    private void interestTask() {
        if (this.plugin.config().getBoolean("Settings.interestEnabled").booleanValue()) {
            BankSystem.logger.log(Level.INFO, "Interest will be given every {0} minute(s).", this.plugin.config().getInteger("Settings.interestCooldown"));
            BukkitTask task = Bukkit.getScheduler().runTaskTimerAsynchronously(this.plugin, new Runnable() {
                public void run() {
                    InterestHandler.this.lastInterestTime = System.currentTimeMillis();
                    List<Player> onlinePlayers = new ArrayList<Player>(Bukkit.getOnlinePlayers());
                    if (!onlinePlayers.isEmpty()) {
                        for (Player p : onlinePlayers) {
                            Double intPercentage = Double.valueOf(Double.parseDouble(InterestHandler.this.plugin.config().getString("Settings.interestPercentage").replace("%", "")));
                            Double balance = InterestHandler.this.plugin.getMoneyDatabaseInterface().getBalance(p);
                            if (balance.doubleValue() > 0.0D && balance.doubleValue() < InterestHandler.this.plugin.config().getInteger("Settings.bankLimit").intValue()) {
                                ...
                                if (interest.doubleValue() > 0.01D) {
                                    ...
                                    InterestHandler.this.plugin.config().printMessage(p, "Messages.interest", interest.toString(), p, p.getName(), Boolean.valueOf(true));
                                }
                            }
                        }
                        onlinePlayers.clear();
            ...
            }, 20L, this.plugin.config().getInteger("Settings.interestCooldown").intValue() * 1200L);
